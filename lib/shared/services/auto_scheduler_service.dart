...
class AutoSchedulerService {
  ...
  void start() async {
    ...
    _notificationResponseSubscription = _notificationService.onResponse.listen((response) {
      _handleNotificationResponse(familyId, response);
    });

    _scheduleSubscription?.cancel();
    _scheduleSubscription = _watchFamilySchedulesUseCase.call(familyId).listen((schedules) async {
       await _rescheduleAll(user.uid, familyId, schedules);
    });
  }

  Future<void> _handleNotificationResponse(String familyId, dynamic response) async {
    ...
    try {
      ...
      debugPrint('Đã ghi log uống thuốc thành công từ thông báo.');
    } catch (e) {
      debugPrint('Lỗi khi ghi log từ thông báo: $e');
    }
  }

  Future<void> _rescheduleAll(String currentUserId, String familyId, List<PatientSchedule> schedules) async {
    await _notificationService.cancelAll();

    final healthProfile = await _getHealthProfileUseCase.call(params: currentUserId);
    final anchorTimes = healthProfile?.anchorTimes ?? const AnchorTimes();

    final mySchedules = schedules.where((s) => s.targetUserId == currentUserId).toList();

    for (var i = 0; i < mySchedules.length; i++) {
        final schedule = mySchedules[i];
        final baseTimeStr = _getAnchorTimeStr(schedule.timing['anchor_event'] as String?, anchorTimes);
        ...
        for (int day = 0; day < 7; day++) {
            ...
            if (scheduledDate.isAfter(now)) {
                final notificationId = (schedule.id.hashCode + day).abs() % 2147483647;
                
                await _notificationService.scheduleNotification(
                    id: notificationId,
                    title: 'Nhắc nhở uống thuốc',
                    body: 'Đã đến lúc uống ${schedule.medName} (${schedule.dosage})',
                    scheduledDate: scheduledDate,
                    payload: '${schedule.id}|${scheduledDate.toIso8601String()}',
                );
            }
        }
    }
